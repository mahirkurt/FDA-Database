...
async def lifespan(app: FastAPI):
    ...
    # Cloudflare bot korumasını aşmak için kendimizi tarayıcı gibi tanıtacak başlık bilgisi
    storage_options = {
        'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/91.0.4472.124 Safari/537.36'
    }

    print(f"Veri seti buluttan (verimli streaming yöntemiyle) yükleniyor: {DATA_URL}")
    
    try:
        # Pandas'a dosyayı okurken bu başlıkları kullanmasını söylüyoruz.
        # Bu yöntem, dosyayı parça parça okur ve belleği doldurmaz.
        app.state.df = pd.read_parquet(DATA_URL, storage_options=storage_options)
